...
func main() {
	...
	err = api.GET(router, doc, core.Route("/auth/apikey/profile", core.Operation{
		...
		if r.Header.Get("X-API-Key") != sampleAPIKey {
			w.WriteHeader(http.StatusUnauthorized)
			_ = json.NewEncoder(w).Encode(map[string]string{"error": "invalid or missing X-API-Key"})
			return
		}
	...
	err = api.GET(router, doc, core.Route("/auth/basic/profile", core.Operation{
		...
		if !checkBasicAuth(r.Header.Get("Authorization")) {
			w.Header().Set("WWW-Authenticate", `Basic realm="docs-example"`)
			w.WriteHeader(http.StatusUnauthorized)
			_ = json.NewEncoder(w).Encode(map[string]string{"error": "invalid or missing basic auth credentials"})
			return
		}
		_ = json.NewEncoder(w).Encode(map[string]any{
	...
	err = api.GET(router, doc, core.Route("/auth/bearer/profile", core.Operation{
		...
		if !checkBearerAuth(r.Header.Get("Authorization"), sampleBearerToken) {
			w.WriteHeader(http.StatusUnauthorized)
	...
	err = api.GET(router, doc, core.Route("/auth/mtls/profile", core.Operation{
		...
		_ = json.NewEncoder(w).Encode(map[string]any{"subject": "docs-only", "authType": "mutualTLS", "scopes": []string{}, "issuedFor": "docs-example"})
	})
	if err != nil {
		log.Fatal(err)
	}

	err = api.GET(router, doc, core.Route("/auth/oidc/profile", core.Operation{
		...
		_ = json.NewEncoder(w).Encode(map[string]any{"subject": "docs-only", "authType": "openIdConnect", "scopes": []string{}, "issuedFor": "docs-example"})
	})
	if err != nil {
		log.Fatal(err)
	}
	...
	err = api.GET(router, doc, core.Route("/auth/oauth/session", core.Operation{
		...
		if !checkBearerAuth(r.Header.Get("Authorization"), sampleOAuthToken) {
			w.WriteHeader(http.StatusUnauthorized)
			_ = json.NewEncoder(w).Encode(map[string]string{"error": "invalid or missing oauth2 bearer token"})
			return
		}
		_ = json.NewEncoder(w).Encode(map[string]any{
			"subject":   "user_123",
